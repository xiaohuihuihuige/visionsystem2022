...
class FXProtocol : public QObject
{
    Q_OBJECT
public:
    explicit FXProtocol(QObject *parent = nullptr);
    //析构函数
    ~FXProtocol();
    ...
    //初始化参数
    void initPar(QString comname,QString portName, QString parity, int baud, int databits, int stopbit,int readspace,bool readsignal,QMap<QString,int> addrInfo);
    //初始化通信
    int8_t initFXProtocol();
    //断开链接
    bool disconnectFXProtocol();
    //发送数据
    void sendHex(QString sendText);
    //计算校验和
    QString computeSum(QString linkstring);
    void readBit(int startAddr,int num);
    void writeBit(int startAddr, QList<int> writedata);
    void readWord(int startAddr, int num);
    void writeWord(int startAddr, QList<int> writedata);

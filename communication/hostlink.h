...
class HostLink : public QObject
{
    Q_OBJECT
public:
    explicit HostLink(QObject *parent = nullptr);
    ~HostLink();
    ...
    int8_t initHostLink();
    bool disconnectHostLink();
    void sendStr(QString sendText);

    QString computeFCS(QString linkstring);
    void writeHostlinkCIOBit(int startAddr, int bit, QList<int> dataWrite);
    void readHostlinkCIOBit(int startAddr, int bit, int count);
    void writeHostlinkCIOWord(int startAddr, QList<int> dataWrite);
    void readHostlinkCIOWord(int startAddr, int count);
    void writeHostlinkWBit(int startAddr, int bit, QList<int> dataWrite);
    void writeHostlinkWWord(int startAddr,  QList<int> dataWrite);
    void readHostlinkWBit(int startAddr, int bit, int count);
    void readHostlinkWWord(int startAddr, int count);

...
class WeiKongPLC : public QObject
{
    Q_OBJECT
public:
    explicit WeiKongPLC(QObject *parent = nullptr);
    ~WeiKongPLC();
    ...
    //初始化参数
    void initPar(QString comname,int serverID, QString portName, QString parity, int baud,
                      int databits, int stopbit,int readspace,bool readsignal,QMap<QString,int> addrInfo);
    //初始化通信
    int8_t initWeiKongPLC();
    //断开链接
    bool disconnectWeiKongPLC();
    //发送数据
    void sendHex(QString sendText);
    //计算校验和
    QString crc16(QString linkstring);

...
struct Feature
{
    ...
    Feature() : x(0), y(0), a(0), label(0) {}
    Feature(int x, int y, int a, int label);
};
inline Feature::Feature(int _x, int _y, int _a, int _label) : x(_x), y(_y), a(_a), label(_label) {}
...
public:
    ...
    struct Candidate
    {
        Candidate(int x, int y, int a, int label, float score);
...
inline ColorGradientPyramid::Candidate::Candidate(int x, int y, int a, int label, float _score) : f(x, y, a , label), score(_score) {}
...
public:
	...
	Detector(int num_features, std::vector<int> T, float weak_threshold, float strong_threshold);//新增阈值设置重载函数
    ...
    int addTemplate(const cv::Mat sources, const std::string &class_id,
                                    const cv::Mat &object_mask, float angle,int nms_kernel_size, int num_features = 0);
...
public:
    ...
    shapeInfo(cv::Mat src, cv::Mat mask = cv::Mat()){
        ...
        if(mask.empty()){
            ...
            this->mask = cv::Mat(src.size(), CV_8UC1, cv::Scalar(255,255,255));
        }else{
            this->mask = mask;
        }
    }
    ...
    void produce_infos(){
		...
		if(angle_range.size() == 1 && scale_range.size() == 1){
			...
			infos.emplace_back(shape_and_info(src_transformed, mask_transformed, angle, scale));
			/*infos.emplace_back(src_transformed);
			infos.emplace_back(mask_transformed);
			infos.emplace_back(angle);
			infos.emplace_back(scale);*/

#include "pro_meltplugrowcheck.h"
...
Pro_MeltPlugRowCheck::Pro_MeltPlugRowCheck(QObject *parent) : QObject(parent)
{
...
Pro_MeltPlugRowCheck::~Pro_MeltPlugRowCheck()
{

}
//初始化参数
void Pro_MeltPlugRowCheck::initGlobalPar(QString pro,QString station,QString cam,QString step,QString type)
{
    m_ProName=pro;
...
void Pro_MeltPlugRowCheck::openHalconWindow()
{
...
//根据启用的品类，读取选区信息
void Pro_MeltPlugRowCheck::readROI()
{
...
//读取品种文件
void Pro_MeltPlugRowCheck::readTypeInfo()
{

}
//根据启用的品类读取参数信息
void Pro_MeltPlugRowCheck::readPar()
{
...
//计算点击处的像素信息
void Pro_MeltPlugRowCheck::slot_press_point(QString cam,QPoint point, QSize size)
{
    if(cam!=m_CamName)
        return;
    try
...
//接收一帧
void Pro_MeltPlugRowCheck::slot_one_frame(QImage src,QString camname,bool ischeck,QString step)
{
...
//检测
void Pro_MeltPlugRowCheck::slot_check(QString camName,QString step)
{
    if(camName!=m_CamName||step!=m_StepName)return;
    if(QtHalcon::testHObjectEmpty(m_SceneImage))return;
    ...
    while(iter!=map_hobj.end())
    {
        ...
        try
        {
            ...
            HTuple hv_is_have,hv_angle_deg;
            HObject ho_ResultContour_2,m_line_melt,m_arrow_distance;
            check_plugrow (m_SceneImage, value, &ho_ResultContour_2,
                           m_IntParameter[QString::fromLocal8Bit("边缘强度")],
                    m_IntParameter[QString::fromLocal8Bit("由黑到白")],
                    m_IntParameter[QString::fromLocal8Bit("第一条边")],
                    m_FloatParameter[QString::fromLocal8Bit("角度小")],
                    &hv_is_have, &hv_angle_deg);



            QString text;
            text=QString::fromLocal8Bit("角度:")+QString::number(hv_angle_deg.D());
...
//读取文件
void Pro_MeltPlugRowCheck::slot_read_image(QString cam,QString step,QString imgPath)
{
...
//写入记录
void Pro_MeltPlugRowCheck::writeImageAndReport()//写入图片和记录
{
...
//写或绘制检测信息到虚拟窗口
void Pro_MeltPlugRowCheck::writeCheckInfoToImage()//写检测结果到图片
{
    try
    {
...
//发送结果
void Pro_MeltPlugRowCheck::send_result()
{
...
//选区变化
void Pro_MeltPlugRowCheck::slot_roi_change(QString cam,QString step)
{
...
//品种变化
void Pro_MeltPlugRowCheck::slot_type_change(QString type)
{
    m_TypeName=type;
    m_TypePath=m_StepPath+"\\"+m_TypeName;
    if(!GeneralFunc::isDirExist(m_TypePath,false))return;
...
//参数变化
void Pro_MeltPlugRowCheck::slot_par_change(QString cam,QString step)
{
    if(cam==m_CamName&&m_StepName==step)
    {
        readPar();
...
//程序结束
void Pro_MeltPlugRowCheck::slot_program_exit()
{

}
void Pro_MeltPlugRowCheck::check_plugrow (HObject ho_Image, HObject ho_ROI, HObject *ho_ResultContour_2,
                                          ...
                                          HTuple *hv_is_have, HTuple *hv_angle_deg)
{
    ...
    HObject  ho_ImageReduced1, ho_Edge_Cross;

    // Local control variables
    HTuple  hv_Row13, hv_Column13, hv_Row23, hv_Column23;
    HTuple  hv_Line2, hv_Length, hv_line1, hv_Angle;

    (*hv_is_have) = 0;
    (*hv_angle_deg) = 0;
    GenEmptyObj(&(*ho_ResultContour_2));
    SmallestRectangle1(ho_ROI, &hv_Row13, &hv_Column13, &hv_Row23, &hv_Column23);
    ReduceDomain(ho_Image, ho_ROI, &ho_ImageReduced1);
    QtHalcon::findLineInRectangle1(ho_ImageReduced1, &(*ho_ResultContour_2), &ho_Edge_Cross,
                                   hv_edge_strength, hv_Black_To_White, hv_First_Edge, ((hv_Row13.TupleConcat(hv_Column13)).TupleConcat(hv_Row23)).TupleConcat(hv_Column23),
                                   1, &hv_Line2);
    TupleLength(hv_Line2, &hv_Length);
    if (0 != (hv_Length==4))
    {
        hv_line1.Clear();
        hv_line1.Append(HTuple(hv_Line2[0]));
        hv_line1.Append(HTuple(hv_Line2[1]));
        hv_line1.Append(HTuple(hv_Line2[0]));
        hv_line1.Append(HTuple(hv_Line2[1])+100);
        AngleLl(HTuple(hv_Line2[0]), HTuple(hv_Line2[1]), HTuple(hv_Line2[0]), HTuple(hv_Line2[1])+100,
                HTuple(hv_Line2[0]), HTuple(hv_Line2[1]), HTuple(hv_Line2[2]), HTuple(hv_Line2[3]),
                &hv_Angle);
        (*hv_angle_deg) = hv_Angle.TupleDeg();
        if (0 != (((*hv_angle_deg).TupleAbs())<hv_angle_min))
        {
            (*hv_is_have) = 1;
        }
    }

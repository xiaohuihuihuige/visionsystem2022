...
Pro_MetalCheck::Pro_MetalCheck(QObject *parent) : QObject(parent)
{
    map_openvino.clear();
    map_Thread.clear();
...
Pro_MetalCheck::~Pro_MetalCheck()
{
    slot_program_exit();
}
//初始化参数
void Pro_MetalCheck::initGlobalPar(QString pro,QString station,QString cam,QString step,QString type)
{
    m_ProName=pro;
...
void Pro_MetalCheck::openHalconWindow()
{
...
//读取品种文件
void Pro_MetalCheck::readTypeInfo()
{
    //slot_program_exit();
    map_Thread.clear();
    if(m_StringParameter[QString::fromLocal8Bit("处理器")]=="GPU")
    {
        ...
        else
        {
            for (int i=0;i<map_hobj.size();i++)
            {
                ...
                connect(this,&Pro_MetalCheck::signal_check_image,map_Classified[i],&Classified::slot_run);
                connect(this,&Pro_MetalCheck::signal_setCheckPar,map_Classified[i],&Classified::slot_setCheckPar);
                connect(map_Classified[i],&Classified::signal_result,this,&Pro_MetalCheck::slot_check_result);
                map_Thread[i]->start();
            }
        }
    }
    else if(m_StringParameter[QString::fromLocal8Bit("处理器")]=="CPU")
    {
        ...
        else
        {
            for (int i=0;i<map_hobj.size();i++)
            {
                ...
                connect(this,&Pro_MetalCheck::signal_check_image,map_openvino[i],&classfiedopenvino::slot_run);
                connect(this,&Pro_MetalCheck::signal_setCheckPar,map_openvino[i],&classfiedopenvino::slot_setCheckPar);
                connect(map_openvino[i],&classfiedopenvino::signal_result,this,&Pro_MetalCheck::slot_check_result);
                map_Thread[i]->start();
            }
        }
    }
...
//计算点击处的像素信息
void Pro_MetalCheck::slot_press_point(QString cam,QPoint point, QSize size)
{
    if(cam!=m_CamName)
        return;
    try
...
//接收一帧
void Pro_MetalCheck::slot_one_frame(QImage src,QString camname,bool ischeck,QString step)
{
...
//检测
void Pro_MetalCheck::slot_check(QString camName,QString step)
{
    if(camName!=m_CamName||step!=m_StepName)return;
    if(QtHalcon::testHObjectEmpty(m_SceneImage))return;
...
void Pro_MetalCheck::slot_check_result(int num,QString name)
{
...
//读取文件
void Pro_MetalCheck::slot_read_image(QString cam,QString step,QString imgPath)
{
...
//写入记录
void Pro_MetalCheck::writeImageAndReport()//写入图片和记录
{
...
//写或绘制检测信息到虚拟窗口
void Pro_MetalCheck::writeCheckInfoToImage()//写检测结果到图片
{
    try
    {
        ...
        if(v_obj_bool.contains(2)||v_obj_bool.contains(3))
        {
...
//发送结果
void Pro_MetalCheck::send_result()
{
    emit signal_result(m_send_data,m_CamName,m_StepName);
}
//选区变化
void Pro_MetalCheck::slot_roi_change(QString cam,QString step)
{
...
//品种变化
void Pro_MetalCheck::slot_type_change(QString type)
{
    m_TypeName=type;
    m_TypePath=m_StepPath+"\\"+m_TypeName;
    if(!GeneralFunc::isDirExist(m_TypePath,false))return;
...
//参数变化
void Pro_MetalCheck::slot_par_change(QString cam,QString step)
{
    if(cam==m_CamName&&m_StepName==step)
    {
        readPar();
...
//程序结束
void Pro_MetalCheck::slot_program_exit()
{

...
Pro_PipeDetect::Pro_PipeDetect(QObject *parent) : QObject(parent)
  ,m_openvino(nullptr)
{
...
Pro_PipeDetect::~Pro_PipeDetect()
{
    slot_program_exit();
}
void Pro_PipeDetect::initGlobalPar(QString pro,QString station,QString cam,QString step,QString type)
{
    m_ProName=pro;
...
void Pro_PipeDetect::openHalconWindow()
{
...
void Pro_PipeDetect::readTypeInfo()
{
...
void Pro_PipeDetect::slot_press_point(QString cam,QPoint point, QSize size)
{
    if(cam!=m_CamName)
        return;
    try
    ...
    {
        emit  signal_information_text(m_CamName,1,"halcon"+QString::fromStdString(e.what()));
    }
}
void Pro_PipeDetect::slot_one_frame(QImage src,QString camname,bool ischeck,QString step)
{
...
void Pro_PipeDetect::slot_check(QString camName,QString step)
{
    if(camName!=m_CamName||step!=m_StepName)return;
    if(QtHalcon::testHObjectEmpty(m_SceneImage))return;
    ...
    try
    {
        HObject ho_Rectangle,ho_ImagePart,image_reduce;
        ReduceDomain(m_SceneImage,map_hobj["roi0#Rectangle1"],&image_reduce);
        getCheckRegion (image_reduce, &ho_Rectangle,&ho_ImagePart,
                        m_FloatParameter[QString::fromLocal8Bit("阈值最大值")],
                m_FloatParameter[QString::fromLocal8Bit("图像宽度")]);
        ...
        v_obj_bool<<type.toInt();
        if(type.toInt()==1)
        {
            HObject ho_heightRectangele;
            HTuple hv_insert_height,hv_is_height_right;
            get_pipeinsert_height(m_SceneImage,v_obj_result[0],&ho_heightRectangele,m_FloatParameter[QString::fromLocal8Bit("金属器宽度")],
                    m_FloatParameter[QString::fromLocal8Bit("像素宽度")],m_FloatParameter[QString::fromLocal8Bit("插管高度小")],
                    &hv_insert_height,&hv_is_height_right);
            if(hv_insert_height.D()>0.0)
            {
                v_obj_result<<ho_heightRectangele;
                v_str_result<<QString::number(hv_insert_height.D(),'f',3);
            }
            if(hv_is_height_right.I()==0)
            {
                v_obj_bool[0]=2;
                v_obj_bool<<2;
                v_str_bool<<2;
        ...
        {
            HTuple hv_result_width,hv_ispipe;
            check_pipe (v_result_image[0], m_FloatParameter[QString::fromLocal8Bit("插管宽度")],
                    m_FloatParameter[QString::fromLocal8Bit("插管公差")],
                    &hv_result_width, &hv_ispipe);
            if(hv_ispipe.I()==1)
            {
                v_obj_bool[0]=7;
            }
            if(hv_result_width.TupleLength().I()>0)
            {
                m_runInfo=tr(QString::fromLocal8Bit("%1插管宽度:%2").arg(m_CamName,QString::number(hv_result_width.D(),'f',2)).toUtf8());
    ...
    v_str_result<<QString::number(v_obj_bool[0]);
    v_str_bool<<int(v_obj_bool[0]==1?1:2);
    if(v_obj_bool[0]==2)
        ng_num++;
    else if(v_obj_bool[0]==1)
        ng_num=0;
    if(m_IntParameter[QString::fromLocal8Bit("地址拍照复位")]==1)
        m_send_data<<0;

    if(m_IntParameter[QString::fromLocal8Bit("地址完成置位")]==1)
        m_send_data<<1;

    m_send_data<<v_obj_bool[0];

    if(m_IntParameter[QString::fromLocal8Bit("地址计数")]==1)
        m_send_data<<ng_num;
    ...
    m_runInfo=QString::fromLocal8Bit("%1检测结果:%2").arg(m_CamName,QString::number(v_obj_bool[0]));
    emit  signal_information_text(m_CamName,1,m_runInfo);
...
void Pro_PipeDetect::slot_check_result(int num,QString name)
{
}
void Pro_PipeDetect::slot_read_image(QString cam,QString step,QString imgPath)
{
...
void Pro_PipeDetect::writeImageAndReport()//写入图片和记录
{
...
void Pro_PipeDetect::writeCheckInfoToImage()//写检测结果到图片
{
    try
    {
        ...
        SetLineWidth(hv_WindowHandle_UI,m_IntParameter[QString::fromLocal8Bit("绘制边缘宽度")]);
...
void Pro_PipeDetect::send_result()
{
    emit signal_result(m_send_data,m_CamName,m_StepName);
}
//选区变化
void Pro_PipeDetect::slot_roi_change(QString cam,QString step)
{
...
//品种变化
void Pro_PipeDetect::slot_type_change(QString type)
{
    m_TypeName=type;
    m_TypePath=m_StepPath+"\\"+m_TypeName;
    if(!GeneralFunc::isDirExist(m_TypePath,false))return;
...
//参数变化
void Pro_PipeDetect::slot_par_change(QString cam,QString step)
{
    if(cam==m_CamName&&m_StepName==step)
    {
        readPar();
...
void Pro_PipeDetect::slot_program_exit()
{
    if(m_openvino!=nullptr)
    {
        delete m_openvino;
...
void Pro_PipeDetect::getCheckRegion (HObject ho_Image1, HObject *ho_Rectangle, HObject *ho_ImagePart,
                                     HTuple hv_maxThr, HTuple hv_width)
{
    ...
    Threshold(ho_Image1, &ho_Region, 0, hv_maxThr);
    AreaCenter(ho_Region, &hv_Area, &hv_Row, &hv_Column);
    GenRectangle1(&(*ho_Rectangle), 0, hv_Column-hv_width, 2048, hv_Column+hv_width);
    ReduceDomain(ho_Image1, (*ho_Rectangle), &ho_ImageReduced);
    CropDomain(ho_ImageReduced, &(*ho_ImagePart));
    return;
}
void Pro_PipeDetect::check_pipe (HObject ho_Image1, HTuple hv_std_width, HTuple hv_off_width, HTuple *hv_result_width,
                                 HTuple *hv_ispipe)
{
    ...
    // Local control variables
    HTuple  hv_Width1, hv_Height, hv_UsedThreshold;
    HTuple  hv_Row1, hv_Column1, hv_Phi, hv_Length11, hv_Length21;

    (*hv_ispipe) = 0;
    GetImageSize(ho_Image1, &hv_Width1, &hv_Height);
    GenRectangle1(&ho_Rectangle, 0, 0, hv_Height*0.75, hv_Width1);
    ReduceDomain(ho_Image1, ho_Rectangle, &ho_ImageReduced);
    BinaryThreshold(ho_ImageReduced, &ho_Region, "max_separability", "dark", &hv_UsedThreshold);
    SmallestRectangle2(ho_Region, &hv_Row1, &hv_Column1, &hv_Phi, &hv_Length11, &hv_Length21);
    (*hv_result_width) = hv_Length21*2;
    if (0 != (HTuple((*hv_result_width)>(hv_std_width-hv_off_width)).TupleAnd((*hv_result_width)<(hv_std_width+hv_off_width))))
    {
        (*hv_ispipe) = 1;
    }
    return;
}
void Pro_PipeDetect::get_pipeinsert_height (HObject ho_Image, HObject ho_ROI, HObject *ho_heightRectangele,
                                            HTuple hv_metal_width, HTuple hv_pixel_width, HTuple hv_min_insert_height, HTuple *hv_insert_height,
                                            HTuple *hv_is_height_right)
{

    // Local iconic variables
    HObject  ho_ImageReduced3, ho_Region3, ho_Rectangle1;
    HObject  ho_RegionClosing1, ho_regionopeing, ho_ConnectedRegions;
    HObject  ho_SelectedRegions, ho_RegionUnion;

    // Local control variables
    HTuple  hv_UsedThreshold2, hv_Row2, hv_Column2;
    HTuple  hv_Phi2, hv_Length12, hv_Length22;

    (*hv_insert_height) = 0;
    (*hv_is_height_right) = 0;
    ReduceDomain(ho_Image, ho_ROI, &ho_ImageReduced3);
    BinaryThreshold(ho_ImageReduced3, &ho_Region3, "max_separability", "dark", &hv_UsedThreshold2);
    GenRectangle2(&ho_Rectangle1, 1024, 1224, 0, hv_metal_width/2, 1);
    Closing(ho_Region3, ho_Rectangle1, &ho_RegionClosing1);
    Opening(ho_RegionClosing1, ho_Rectangle1, &ho_regionopeing);
    Connection(ho_regionopeing, &ho_ConnectedRegions);
    SelectShape(ho_ConnectedRegions, &ho_SelectedRegions, "height", "and", (hv_min_insert_height/hv_pixel_width)/2,
                99999);
    Union1(ho_SelectedRegions, &ho_RegionUnion);
    SmallestRectangle2(ho_RegionUnion, &hv_Row2, &hv_Column2, &hv_Phi2, &hv_Length12,
                       &hv_Length22);
    GenRectangle2(&(*ho_heightRectangele), hv_Row2, hv_Column2, hv_Phi2, hv_Length12,
                  hv_Length22);
    if (0 != (((hv_Phi2.TupleDeg()).TupleAbs())>45))
    {
        (*hv_insert_height) = (hv_Length12*2)*hv_pixel_width;
    }
    else
    {
        (*hv_insert_height) = (hv_Length22*2)*hv_pixel_width;
    }

    if (0 != ((*hv_insert_height)>hv_min_insert_height))
    {
        (*hv_is_height_right) = 1;
    }
    else
    {
        (*hv_is_height_right) = 0;

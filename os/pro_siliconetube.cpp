...
//初始化参数
void Pro_SiliconeTube::initGlobalPar(QString pro,QString station,QString cam,QString step,QString type)
{
    m_ProName=pro;
...
void Pro_SiliconeTube::openHalconWindow()
{
...
//接收一帧
void Pro_SiliconeTube::slot_one_frame(QImage src,QString camname,bool ischeck,QString step)
{
...
//检测
void Pro_SiliconeTube::slot_check(QString camName,QString step)
{
    if(camName!=m_CamName||step!=m_StepName)return;
    if(QtHalcon::testHObjectEmpty(m_SceneImage))return;
    ...
    while(iter!=map_hobj.end())
    {
        ...
        try
        {
            ...
            HTuple hv_Area2,hv_is_have;
            HObject ho_RegionUnion,check_rectangle;

            checkSilicone (m_SceneImage, value,&check_rectangle, &ho_RegionUnion,
                           m_FloatParameter[QString::fromLocal8Bit("导管座阈值")], m_FloatParameter[QString::fromLocal8Bit("导管座开运算宽")],
                    m_FloatParameter[QString::fromLocal8Bit("导管座开运算高")], m_FloatParameter[QString::fromLocal8Bit("孔检测直径")],
                    m_FloatParameter[QString::fromLocal8Bit("孔阈值")], m_FloatParameter[QString::fromLocal8Bit("孔最小面积")],
                    m_FloatParameter[QString::fromLocal8Bit("孔位置")],&hv_Area2, &hv_is_have);
            v_obj_result.push_back(check_rectangle);
            v_obj_bool.push_back(1);

            int a=hv_Area2.TupleLength();
            QString text;
            text=QString::fromLocal8Bit("面积:")+QString::number(a>0?hv_Area2.I():0);
            v_str_result.push_back(text);
            v_obj_result.push_back(ho_RegionUnion);
            if(hv_is_have.I()==1)
            {
                v_obj_bool.push_back(1);
...
//读取文件
void Pro_SiliconeTube::slot_read_image(QString cam,QString step,QString imgPath)
{
...
//选区变化
void Pro_SiliconeTube::slot_roi_change(QString cam,QString step)
{
...
//参数变化
void Pro_SiliconeTube::slot_par_change(QString cam,QString step)
{
    if(cam==m_CamName&&m_StepName==step)
    {
        readPar();
...
void Pro_SiliconeTube::checkSilicone (HObject ho_Image, HObject ho_ROI, HObject *ho_seat_region, HObject *ho_hole_region,
                                      ...
                                      HTuple *hv_is_have)
{
    ...
    HObject  ho_GrayImage, ho_ImageReduced, ho_Region2;
    HObject  ho_RegionOpening, ho_Circle, ho_ImageReduced1, ho_Region3;
    HObject  ho_ConnectedRegions, ho_SelectedRegions;

    // Local control variables
    HTuple  hv_Row1, hv_Column1, hv_Row2, hv_Column2;
    HTuple  hv_Area, hv_Row3, hv_Column3, hv_Row, hv_Column;
    ...
    //计算导管座位置
    ReduceDomain(ho_GrayImage, ho_ROI, &ho_ImageReduced);
    Threshold(ho_ImageReduced, &ho_Region2, hv_seat_thr, 255);
    OpeningRectangle1(ho_Region2, &ho_RegionOpening, hv_seat_open_size_w, hv_seat_open_size_h);
    SmallestRectangle1(ho_RegionOpening, &hv_Row1, &hv_Column1, &hv_Row2, &hv_Column2);
    GenRectangle1(&(*ho_seat_region), hv_Row1, hv_Column1, hv_Row2, hv_Column2);
    //计算孔位置
    GenCircle(&ho_Circle, (hv_Row1+hv_Row2)/2, hv_Column1+hv_hole_pos, hv_hole_radius/2);
    ReduceDomain(ho_GrayImage, ho_Circle, &ho_ImageReduced1);
    Threshold(ho_ImageReduced1, &ho_Region3, 0, hv_hole_thr);
    Connection(ho_Region3, &ho_ConnectedRegions);
    AreaCenter(ho_ROI, &hv_Area, &hv_Row3, &hv_Column3);
    SelectShape(ho_ConnectedRegions, &ho_SelectedRegions, "area", "and", hv_hole_min_area,
                hv_Area);
    Union1(ho_SelectedRegions, &(*ho_hole_region));
    AreaCenter((*ho_hole_region), &(*hv_hole_area), &hv_Row, &hv_Column);
    ConcatObj((*ho_hole_region), ho_Circle, &(*ho_hole_region));
    if (0 != ((*hv_hole_area)<1))
    {
        (*hv_is_have) = 1;
    }

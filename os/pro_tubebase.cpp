...
Pro_TubeBase::Pro_TubeBase(QObject *parent) : QObject(parent),
    m_openvino(nullptr)
{
...
Pro_TubeBase::~Pro_TubeBase()
{
    slot_program_exit();
}
//初始化参数
void Pro_TubeBase::initGlobalPar(QString pro,QString station,QString cam,QString step,QString type)
{
    m_ProName=pro;
...
void Pro_TubeBase::openHalconWindow()
{
...
//读取品种文件
void Pro_TubeBase::readTypeInfo()
{
    slot_program_exit();
...
//计算点击处的像素信息
void Pro_TubeBase::slot_press_point(QString cam,QPoint point, QSize size)
{
    if(cam!=m_CamName)
        return;
    try
...
//接收一帧
void Pro_TubeBase::slot_one_frame(QImage src,QString camname,bool ischeck,QString step)
{
...
//检测
void Pro_TubeBase::slot_check(QString camName,QString step)
{
    if(camName!=m_CamName||step!=m_StepName)return;
    if(QtHalcon::testHObjectEmpty(m_SceneImage))return;
    ...
    while(iter!=map_hobj.end())
    {
        ...
        try
        {
            ...
            HObject ho_seat_rect,ho_pipe_rect,ho_ImagePart;
            HTuple hv_type_is,hv_width,hv_height;

            check_seat_and_get_pipe (m_SceneImage,value, &ho_seat_rect,
                                     &ho_pipe_rect, &ho_ImagePart,
                                     m_IntParameter[QString::fromLocal8Bit("导管座宽度小")],
                    m_IntParameter[QString::fromLocal8Bit("导管座高度大")],
                    m_IntParameter[QString::fromLocal8Bit("检测导管")],
                    m_IntParameter[QString::fromLocal8Bit("导管宽度")],
                    m_IntParameter[QString::fromLocal8Bit("导管检测尺寸")],
                    m_IntParameter[QString::fromLocal8Bit("金属高度")],
                    m_IntParameter[QString::fromLocal8Bit("旋转角度")],
                    m_FloatParameter[QString::fromLocal8Bit("导管座阈值")],
                    m_IntParameter[QString::fromLocal8Bit("偏移距离")],
                    &hv_type_is, &hv_width, &hv_height);
            QString text;
            text=QString::fromLocal8Bit("宽度:")+QString::number(hv_width.I());
            v_str_result.push_back(text);
            text=QString::fromLocal8Bit("高度:")+QString::number(hv_height.I());
            v_str_result.push_back(text);
            v_obj_result.push_back(ho_seat_rect);
            if(hv_type_is.I()==1)
            {
            ...
            if(hv_type_is.I()==1)
            {
                if(m_IntParameter[QString::fromLocal8Bit("检测导管")]==1)
                {
                    ...
                    v_obj_result.push_back(ho_pipe_rect);
                    v_obj_bool.push_back(check_name_list.size()>0?2:1);

                    v_result_image.push_back(result_image);
                    v_result_image_rect.push_back(ho_pipe_rect);

                    text=QString::fromLocal8Bit("结果:")+QString::number(check_name_list.size()>0?4:1);
                    v_str_result.push_back(text);
                    v_str_bool.push_back(check_name_list.size()>0?2:1);

                    roi_result<<(check_name_list.size()>0?4:1);
                }
                else
                {
                    v_obj_result.push_back(value);
                    v_obj_bool.push_back(1);

                    text=QString::fromLocal8Bit("结果:")+QString::number(1);
                    v_str_result.push_back(text);
                    v_str_bool.push_back(1);
            ...
            else
            {
                ...
                text=QString::fromLocal8Bit("结果:")+QString::number(hv_type_is.I());
                v_str_result.push_back(text);
                v_str_bool.push_back(2);

                roi_result<<hv_type_is.I();
            }
        }
        catch (cv::Exception &e)
        {
...
void Pro_TubeBase::slot_check_result(int num,cv::Mat img_result,QStringList name)
{
...
//读取文件
void Pro_TubeBase::slot_read_image(QString cam,QString step,QString imgPath)
{
...
//写入记录
void Pro_TubeBase::writeImageAndReport()//写入图片和记录
{
...
//写或绘制检测信息到虚拟窗口
void Pro_TubeBase::writeCheckInfoToImage()//写检测结果到图片
{
    try
    {
...
//发送结果
void Pro_TubeBase::send_result()
{
    emit signal_result(m_send_data,m_CamName,m_StepName);
}
//选区变化
void Pro_TubeBase::slot_roi_change(QString cam,QString step)
{
...
//品种变化
void Pro_TubeBase::slot_type_change(QString type)
{
    m_TypeName=type;
    m_TypePath=m_StepPath+"\\"+m_TypeName;
    if(!GeneralFunc::isDirExist(m_TypePath,false))return;
...
//参数变化
void Pro_TubeBase::slot_par_change(QString cam,QString step)
{
    if(cam==m_CamName&&m_StepName==step)
    {
        readPar();
...
//程序结束
void Pro_TubeBase::slot_program_exit()
{
    if(m_openvino!=nullptr)
    {
        delete m_openvino;
...
void Pro_TubeBase::check_seat_and_get_pipe (HObject ho_Image, HObject ho_ROI_0, HObject *ho_seat_rect,
                                            ...
                                            HTuple *hv_seat_width, HTuple *hv_seat_height)
{
    ...
    HObject  ho_GrayImage, ho_ImageReduced, ho_Region;
    HObject  ho_RegionOpening, ho_RegionClosing, ho_RegionOpening2;
    HObject  ho_RegionOpening3, ho_Rectangle, ho_ImageResult;
    HObject  ho_ImageReduced1;

    // Local control variables
    HTuple  hv_Row11, hv_Column11, hv_Row21, hv_Column21;
    HTuple  hv_height_roi, hv_Row12, hv_Column12, hv_Row22;
    HTuple  hv_Column22, hv_seat_pipe_height, hv_Row1, hv_Column1;
    HTuple  hv_Row2, hv_Column2;

    (*hv_type_is) = 0;
    (*hv_seat_width) = 0;
    (*hv_seat_height) = 0;
    GenEmptyObj(&(*ho_seat_rect));
    GenEmptyObj(&(*ho_pipe_rect));
    if (0 != (hv_rotate_angle==90))
    {
        //计算检测选区高度
        SmallestRectangle1(ho_ROI_0, &hv_Row11, &hv_Column11, &hv_Row21, &hv_Column21);
        hv_height_roi = (hv_Column21-hv_Column11)-10;
        //计算导管座区域
        Rgb1ToGray(ho_Image, &ho_GrayImage);
        ReduceDomain(ho_GrayImage, ho_ROI_0, &ho_ImageReduced);
        Threshold(ho_ImageReduced, &ho_Region, hv_seat_thr, 255);
        OpeningCircle(ho_Region, &ho_RegionOpening, 3.5);
        ClosingRectangle1(ho_RegionOpening, &ho_RegionClosing, 2, hv_seat_width_min/2);
        OpeningRectangle1(ho_RegionClosing, &ho_RegionOpening2, hv_metal_height, 2);
        //计算导管带导管座区域高度
        SmallestRectangle1(ho_RegionOpening2, &hv_Row12, &hv_Column12, &hv_Row22, &hv_Column22);
        hv_seat_pipe_height = hv_Column22-hv_Column12;
        //计算导管座区域高度
        OpeningRectangle1(ho_RegionOpening2, &ho_RegionOpening3, 2, hv_pipe_width);
        SmallestRectangle1(ho_RegionOpening3, &hv_Row1, &hv_Column1, &hv_Row2, &hv_Column2);
        GenRectangle1(&(*ho_seat_rect), hv_Row1, hv_Column1, hv_Row2, hv_Column2);
        (*hv_seat_width) = hv_Row2-hv_Row1;
        (*hv_seat_height) = hv_Column2-hv_Column1;
        if (0 != (HTuple((*hv_seat_width)>=hv_seat_width_min).TupleAnd(hv_seat_pipe_height>hv_height_roi)))
        {
            if (0 != ((*hv_seat_height)<hv_seat_height_max))
            {
                (*hv_type_is) = 1;
                if (0 != (hv_check_pipe==1))
                {
                    //裁剪导管座上方管区域
                    GenRectangle1(&ho_Rectangle, ((hv_Row1+hv_Row2)/2)-(hv_pipe_width/4), hv_Column2,
                                  ((hv_Row1+hv_Row2)/2)+(hv_pipe_width/4), hv_Column2+hv_offset_dis);
                    PaintRegion(ho_Rectangle, ho_GrayImage, &ho_ImageResult, 255, "fill");
                    GenRectangle1(&(*ho_pipe_rect), ((hv_Row1+hv_Row2)/2)-(hv_check_size/2),
                                  hv_Column2, ((hv_Row1+hv_Row2)/2)+(hv_check_size/2), hv_Column2+hv_check_size);
                    ReduceDomain(ho_ImageResult, (*ho_pipe_rect), &ho_ImageReduced1);
        ...
        {
            if (0 != (hv_seat_pipe_height>hv_height_roi))
            {
                (*hv_type_is) = 2;
            }
            else
    ...
    else if (0 != (hv_rotate_angle==0))
    {
        //计算检测选区高度
        SmallestRectangle1(ho_ROI_0, &hv_Row11, &hv_Column11, &hv_Row21, &hv_Column21);
        hv_height_roi = (hv_Row21-hv_Row11)-10;
        //计算导管座区域
        Rgb1ToGray(ho_Image, &ho_GrayImage);
        ReduceDomain(ho_GrayImage, ho_ROI_0, &ho_ImageReduced);
        Threshold(ho_ImageReduced, &ho_Region, hv_seat_thr, 255);
        OpeningCircle(ho_Region, &ho_RegionOpening, 1.0);
        ClosingRectangle1(ho_RegionOpening, &ho_RegionClosing, hv_seat_width_min/2, 2);
        OpeningRectangle1(ho_RegionClosing, &ho_RegionOpening2, 2, hv_metal_height);
        //计算导管带导管座区域高度
        SmallestRectangle1(ho_RegionOpening2, &hv_Row12, &hv_Column12, &hv_Row22, &hv_Column22);
        hv_seat_pipe_height = hv_Row22-hv_Row12;
        //计算导管座区域高度
        OpeningRectangle1(ho_RegionOpening2, &ho_RegionOpening3, hv_pipe_width, 2);
        SmallestRectangle1(ho_RegionOpening3, &hv_Row1, &hv_Column1, &hv_Row2, &hv_Column2);
        GenRectangle1(&(*ho_seat_rect), hv_Row1, hv_Column1, hv_Row2, hv_Column2);
        (*hv_seat_width) = hv_Column2-hv_Column1;
        (*hv_seat_height) = hv_Row2-hv_Row1;
        if (0 != (HTuple((*hv_seat_width)>=hv_seat_width_min).TupleAnd(hv_seat_pipe_height>hv_height_roi)))
        {
            if (0 != ((*hv_seat_height)<hv_seat_height_max))
            {
                ...
                {
                    //裁剪导管座上方管区域
                    GenRectangle1(&ho_Rectangle, hv_Row1-hv_offset_dis, ((hv_Column1+hv_Column2)/2)-(hv_pipe_width/4),
                                  hv_Row1, ((hv_Column1+hv_Column2)/2)+(hv_pipe_width/4));
                    PaintRegion(ho_Rectangle, ho_GrayImage, &ho_ImageResult, 255, "fill");
                    GenRectangle1(&(*ho_pipe_rect), hv_Row1-hv_check_size, ((hv_Column1+hv_Column2)/2)-(hv_check_size/2),
                                  hv_Row1, ((hv_Column1+hv_Column2)/2)+(hv_check_size/2));
                    ReduceDomain(ho_ImageResult, (*ho_pipe_rect), &ho_ImageReduced1);
                    CropDomain(ho_ImageReduced1, &(*ho_ImagePart));
                }
            }
